...
public class Atm implements IAtm {
        ...
        if (creditCard != null){
            if(pin.length()==4){
                try {
                    ...
                    if(creditCard.IsPinValid(pin)) return true;
                    else return false;
    ...
    public double AccountStatus(IAccount account) {
        if(account != null) return account.AccountStatus();
        else return 0;
    }
    ...
    public boolean ChangePinCard(ICreditCard card, String oldPin, String newPin, String newPinConfirm) {
        if(card != null)
        {
            ...
            {
                if(card.ChangePin(oldPin, newPin, newPinConfirm)) return true;
                else return false;
            }
            else return false;
        }
    ...
    public boolean WithdrawFunds(ICreditCard card, double amount) {
        if(card != null && amount > 0)
        {
            if(card.WithdrawFunds(amount)) return true;
            else return false;
        }
        else return false;
    }
    ...
    public boolean Transfer(ICreditCard card, IAccount accountRecipient, double amount) {
        if(card != null && accountRecipient != null && amount > 0)
        {
            ...
            {
                double saldo = accountRecipient.AccountStatus();
                accountRecipient.DepositFunds(amount);
                ///
                if(accountRecipient.AccountStatus() == saldo + amount) return true;
                else {
                    card.DepositFunds(amount);
                    accountRecipient.WithdrawFunds(amount);
                    return false;
                }
            }
            else return false;
